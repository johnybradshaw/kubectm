...
| File | Description |
|------|-------------|
| `download.go` | Dispatcher routing to provider-specific downloaders (Linode, AWS, GCP, Azure) |
...
| `merge_test.go` | Tests for kubeconfig merging and context conflict resolution |
| `security_test.go` | Tests for path traversal rejection on save and EKS identifier validation |
